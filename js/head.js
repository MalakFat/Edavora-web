...
document.addEventListener('DOMContentLoaded', function() {
    ...
    typeButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            currentRecipients = currentType === 'teachers' ? teachers : students;
    ...
    sendMessageModalBtn.addEventListener('click', function() {
        ...
        selectedRecipients.forEach(recipientName => {
            const recipient = [...teachers, ...students].find(r => r.name === recipientName);
            if (recipient) {
                const existingConversation = conversations.find(c => c.userName === recipientName);
                if (!existingConversation) {
                    const userType = teachers.some(t => t.name === recipientName) ? 'teacher' : 'student';
                    conversations.push({
                        id: Date.now(),
                        userId: recipient.id,
